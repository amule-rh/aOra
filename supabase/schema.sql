...
alter table public.activities add constraint activities_type_check check (type in ('running','cycling','walking','strength','hiit','mobility','swimming','hiking','rowing','trail','elliptical','other')); Route points are kept as JSONB so the browser can
-- save the original timestamp/altitude/accuracy data without losing fidelity.
